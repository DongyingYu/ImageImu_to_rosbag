...
int main(int argc, char **argv)
{
  ...
  while (true)
  {
    ...
    for (size_t i = 0; i < numCameras; ++i)
    {
      ...
      // std::cout << "[INFO]: The number of camera is : " << i << std::endl;
      // get all IMU measurements till then
      double t_imu = start;
      do
      {
        // check if at the end ans read a row of data
...
void open_file(std::string path, std::vector<std::vector<std::string>> &image_names, int numCameras)
{
  ...
  // open image file
  {
    ...
    for (int i = 0; i < numCameras; ++i)
    {
      ...
      // 有参数的directory_iterator(folder)作为迭代器的起点，无参数的作为迭代器的终点;
